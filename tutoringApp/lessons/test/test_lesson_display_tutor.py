"""Tests for the `lesson_display_tutor` page."""
import tempfile
...
class TestLessonDisplayTutor(TestCaseLessonsUtils):
    """Tests for the functionality of displaying Lesson objects from Tutor's perspective."""
    ...
    def test_all_lesson_data_displayed_correctly(self):
        self._login_user(username="tutor1", password="haslo123")
        res = self.client.get(reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}))
    ...
    def test_edit_btn_displayed(self):
        self._login_user(username="tutor1", password="haslo123")
        res = self.client.get(reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}))
    ...
    def test_related_tasks_in_db_no_solution_uploaded_info_displayed(self):
        lesson = Lesson.objects.get(pk=1)
        self._create_task_object(
            lesson=lesson,
            title="Task 1",
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-tasks-main-task-student_solution-info">No solution uploaded</div>""",
            html=True,
        )
    ...
    def test_related_tasks_in_db_no_solution_uploaded_correst_status_displayed(self):
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_red">1 outstanding.</div>""",
            html=True,
        )
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_red">None with rejected solution.</div>""",
            html=True,
        )
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_orange">None to be checked.</div>""",
            html=True,
        )
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom status-entry-bottom_green">None finished.</div>""",
            html=True,
        )
        ...
        self.assertContains(
            ...
            f"""<div class="lesson-left-tasks-main-task-student_solution-file_uploaded">
                        ...
                        <path fill-rule="evenodd" d="M14 4.5V14a2 2 0 0 1-2 2h-1v-1h1a1 1 0 0 0 1-1V4.5h-2A1.5 1.5 0 0 1 9.5 3V1H4a1 1 0 0 0-1 1v9H2V2a2 2 0 0 1 2-2h5.5zM1.6 11.85H0v3.999h.791v-1.342h.803c.287 0 .531-.057.732-.173.203-.117.358-.275.463-.474a1.42 1.42 0 0 0 .161-.677c0-.25-.053-.476-.158-.677a1.176 1.176 0 0 0-.46-.477c-.2-.12-.443-.179-.732-.179Zm.545 1.333a.795.795 0 0 1-.085.38.574.574 0 0 1-.238.241.794.794 0 0 1-.375.082H.788V12.48h.66c.218 0 .389.06.512.181.123.122.185.296.185.522Zm1.217-1.333v3.999h1.46c.401 0 .734-.08.998-.237a1.45 1.45 0 0 0 .595-.689c.13-.3.196-.662.196-1.084 0-.42-.065-.778-.196-1.075a1.426 1.426 0 0 0-.589-.68c-.264-.156-.599-.234-1.005-.234H3.362Zm.791.645h.563c.248 0 .45.05.609.152a.89.89 0 0 1 .354.454c.079.201.118.452.118.753a2.3 2.3 0 0 1-.068.592 1.14 1.14 0 0 1-.196.422.8.8 0 0 1-.334.252 1.298 1.298 0 0 1-.483.082h-.563v-2.707Zm3.743 1.763v1.591h-.79V11.85h2.548v.653H7.896v1.117h1.606v.638H7.896Z"></path>
                                            ...
                                            <div class="lesson-left-tasks-main-task-student_solution-file_uploaded-right adjacent-container " data-solution="1">
    ...
    def test_related_tasks_in_db_solution_uploaded_correct_task_status_displayed(self):
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_red">None outstanding.</div>""",
            html=True,
        )
        self.assertContains(
            res,
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_orange">1 to be checked.</div>""",
            html=True,
        )
        self.assertContains(
            res,
    ...
    def test_related_tasks_in_db_solution_uploaded_and_approved_correct_task_status_displayed(
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom status-entry-bottom_green">1 finished.</div>""",
            html=True,
        )

    @override_settings(MEDIA_ROOT=tempfile.gettempdir())
    def test_related_tasks_in_db_solution_uploaded_and_rejected_correct_task_status_displayed(
        self,
    ):
        self._login_user(username="tutor1", password="haslo123")
        lesson = Lesson.objects.get(pk=1)
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-table-cell-bottom main-table-cell-bottom_red">1 with rejected solution.</div>""",
            html=True,
        )
        self.assertContains(
            res,
    ...
    def test_correct_entry_form_entry_created(self):
        self._login_user(username="tutor1", password="haslo123")
        self.client.post(
            reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}),
            {
                "text": "Entry from Tutor.",
            },
        )
        self.assertEqual(len(Entry.objects.all()), 1)
        self.assertEqual(Entry.objects.get(pk=1).text, "Entry from Tutor.")

    @freeze_time(datetime(2023, 12, 13, 10))
    def test_entry_from_tutor_correctly_displayed(self):
        self._login_user(username="tutor1", password="haslo123")
        ...
        self.assertContains(
            ...
            """<div class="lesson-left-tasks-activity-entries-entry-right-top">
                                        tutor1's first name tutor1's last name, Dec. 13, 2023, 10 a.m.
                                    </div>""",
            html=True,
        )
        self.assertContains(
    ...
    @freeze_time(LESSON_DATE - timedelta(hours=1))
    def test_before_lessons_start_absence_btn_not_displayed(self):
        self._login_user(username="tutor1", password="haslo123")
        res = self.client.get(reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}))
        self.assertNotContains(
            res,
            """<div class="lesson-left-status-main-entry-absence button_fill_red absence-btn" data-lesson="1">
                Student was not present
            </div>
            <div class="lesson-left-status-main-entry-attendance button_fill_green presence-btn hidden" data-lesson="1">
                Student was present
            </div>""",
        )

    @freeze_time(LESSON_DATE + timedelta(days=1))
    def test_after_lessons_start_absence_btns_displayed(self):
        self._login_user(username="tutor1", password="haslo123")
        res = self.client.get(reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}))
        self.assertContains(
            res,
    ...
    @freeze_time(LESSON_DATE + timedelta(days=1))
    def test_after_lessons_start_presence_btns_displayed(
        self,
    ):
        self._login_user(username="tutor1", password="haslo123")
        lesson = Lesson.objects.get(pk=1)
        ...
        self.assertContains(
            res,
            """<div class="lesson-left-status-main-entry-absence button_fill_red absence-btn hidden" data-lesson="1">
                    Student was not present
                </div>
                <div class="lesson-left-status-main-entry-attendance button_fill_green presence-btn" data-lesson="1">
                    Student was present
                </div>""",
            html=True,
        )
    ...
    def test_user_is_not_related_tutor_warning_displayed(self, is_student):
        self._register_user("other_user", student=is_student)
        self.client.login(username="other_user", password="haslo123")
        res = self.client.get(
            reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}), follow=True
        )

        self.assertEqual(res.status_code, 403)

    def test_user_is_related_student_redirected_to_display_page_for_tutors(self):
        self._login_user("student1")
        res = self.client.get(
            reverse("lessons:lesson_display_tutor", kwargs={"pk": 1}), follow=True
        )

        self.assertRedirects(
            res, reverse("lessons:lesson_display_student", kwargs={"pk": 1})
        )
